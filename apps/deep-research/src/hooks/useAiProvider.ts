...
const FALLBACK_GOOGLE_API_KEY = "AIzaSyB2hsXqnoHr5FIIPeC2CxO4phZspnbvzQU";
    ...
    switch (provider) {
      case "google":
        ...
        if (mode === "local") {
          ...
          const googleApiKey = apiKey || FALLBACK_GOOGLE_API_KEY;
          options.apiKey = multiApiKeyPolling(googleApiKey);
        } else {
  ...
  function hasApiKey(): boolean {
    ...
    switch (provider) {
      case "google":
        ...
        return apiKey.length > 0 || FALLBACK_GOOGLE_API_KEY.length > 0;
      case "openai":
